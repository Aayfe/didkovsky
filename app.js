...
function renderEanResolution(product, matches) {
  ...
  aliasInput.setAttribute("list", "product-options");
...
async function detectEanFromImage(file) {
  return await detectEanWithBarcodeDetector(file)
    ...
    || await detectEanWithZxingVariants(file)
...
async function createBarcodeImageVariants(file) {
  try {
    const bitmap = await createImageBitmap(file);
    const maxSide = 2200;
    const scale = Math.min(2.2, Math.max(1, maxSide / Math.max(bitmap.width, bitmap.height)));
    ...
    return [
      await createBarcodeVariantBlob(context, canvas, "contrast"),
      await createBarcodeVariantBlob(context, canvas, "threshold"),
      await createRotatedBarcodeVariantBlob(canvas)
    ].filter(Boolean);
  } catch (error) {
    return [];
  }
...
async function createBarcodeVariantBlob(context, canvas, mode) {
  ...
  for (let index = 0; index < image.data.length; index += 4) {
    ...
    const value = mode === "threshold"
      ? (gray > 145 ? 255 : 0)
      : Math.max(0, Math.min(255, (gray - 128) * 1.85 + 128));
  ...
  nextContext.putImageData(image, 0, 0);
  return await canvasToBlob(nextCanvas);
}

async function createRotatedBarcodeVariantBlob(canvas) {
  ...
  return await canvasToBlob(rotated);
}
...
function upsertCatalogItem(item) {
  ...
  if (nextEans.length) {
    catalogItems.forEach((currentItem) => {
      ...
      const aliases = normalizeEanAliases(currentItem.eanAliases).filter((alias) => !nextEans.includes(alias.ean));
